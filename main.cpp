...
void plane(){
    ...
    /// Main body
    glColor3d(0.5,1,0);  //sets current color
    glPushMatrix();    //pushes the current matrix down by 1 duplicating the curretn mttrix
        glTranslated(0,0,0);
        glScaled(3,0.4,0.5);
        glutSolidSphere(1,30,30);
    glPopMatrix();  //pops the current matrix stack, replacing the current matrix with one below it
...
void singleTolaHouse(int R,int G,int B){
    ...
    glPushMatrix();
        glTranslated(0,0,0);
        glutSolidCube(1);    //draws a solid cube
    glPopMatrix();
...
void house(int n,int R,int G){
    for(int i=0;i<n;i++){
        glPushMatrix();
            glTranslated(0,0.8+i,0);  //multiplies current matrix by translation matrix
...
//controls the environment and rings
...
//to put everything on screen for animation
void draw(){
...
//defined har ek key press karne se kya hota hai
